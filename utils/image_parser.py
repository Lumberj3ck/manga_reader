...
def get_img_in_ch(url: str, img_selector: str):
    print(url)
    resp = get_page(url)
...
def write_into_file_and_save(
    response: Response, path_to_save, path_for_django, chapter_instance
):
...
def create_chapter(chapter_name):
    ...
    return Chapter.objects.get_or_create(
        name=chapter_name, manga=manga_inst, chapter_number=chapter_number
    )


def create_loop(chapter_links, site_url_split):
    for link in chapter_links:
        links = get_img_in_ch(link, img_selector)
...
def main_parser(
    url: str, selector: str, site_url_split: str, amount: int = "all"
) -> None:
